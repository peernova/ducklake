-- Worker A: Moves 'pending' orders to 'processing'
...
CALL ducklake_use_branch('t', 'migrate_a');
SELECT * FROM ducklake_current_branch('t');

SELECT 'MIGRATE_A: Before - partition distribution:';
SELECT status, COUNT(*) as cnt FROM orders GROUP BY status ORDER BY status;

-- Move pending to processing (partition key change!)
UPDATE orders SET status = 'processing' WHERE status = 'pending';
SELECT 'MIGRATE_A: Moved pending -> processing';

SELECT 'MIGRATE_A: After - partition distribution:';
SELECT status, COUNT(*) as cnt FROM orders GROUP BY status ORDER BY status;
